...
bathy <- getNOAA.bathy(110, -130, -35, 25, resolution = 4, antimeridian = TRUE, keep=TRUE)
...
worldmap <-  autoplot(bathy, geom=c('tile'), mapping=NULL, coast = TRUE) +
  ...
  scale_x_continuous(breaks=seq(110,230, by=10), labels=c("110E", "120E", "130E", "140E", "150E", "160E",
                                                          "170E", "180", "170W", "160W", "150W", "140W", "130W"), 
                     expand=c(0,0)) +
  scale_y_continuous(breaks=seq(-30,20,by=10), labels=c("30S", "20S", "10S", "0", "10N", "20N"), expand=c(0,0))

worldmap
worldmapfull <- worldmap + map.layer

data_full_range <- ggplot_build(worldmapfull)$data[[2]]
data_full_range <- data_full_range[data_full_range$x >= 100, ]
data_full_range <- data_full_range[data_full_range$x <= 270, ]
data_full_range <- data_full_range[data_full_range$y >= -45, ]
...
ggsave("~/Desktop/Ongoing Projects/GFW Shark Sanctuary/sanctuarymap.png", sanctuarymap,  width = 12, height = 8, dpi=300)

sanctuarymap <- sanctuarymap %>%
  annotate_figure(fig.lab = "a)")

fig1full <- ggarrange(sanctuarymap,hooks_proj_fig, ncol=1, nrow=2, 
                      labels="auto", heights = c(1,1.3), align="hv")

ggsave("~/Desktop/Ongoing Projects/GFW Shark Sanctuary/Fig1Full.png", 
       fig1full,  width = 8, height = 12, dpi=300)
