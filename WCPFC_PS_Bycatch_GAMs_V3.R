...
data <- read.csv("PS_bycatch.csv")

data <- subset(data, Species.Category == "SHK")
colnames(data)[c(1,3,4,6:12)] = c("Year","Lat","Long","Species","VesselsObserved", "SetsObserved", "Interactions", "InteractionsRate","Mortalities","MortalityRate")
...
ggplot(poly3) +
  geom_sf(color="red") +
  geom_tile(data=data, aes(x=Long2,y=Lat,fill=Interactions)) +
...
#### thresher ####
threshers <- aggregate(data=WCPFC_thresher_data, Interactions ~ Lat + Long2 + Year, FUN='sum')
threshers.m <- aggregate(data=WCPFC_thresher_data, Mortalities ~ Lat + Long2 + Year, FUN='sum')
