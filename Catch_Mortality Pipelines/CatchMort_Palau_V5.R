...
#correct for out of domain cells to closest cell with CPUE
...
{start.time <- Sys.time()
  ...
  palau_bsh_proj$catch.upr <- quantile(palau_bsh_total_catches, 0.95)
  palau_bsh_proj$catch.lwr <- quantile(palau_bsh_total_catches, 0.05)
  
  palau_bsh_total_DOA <- colSums(palau_bsh_DOA)
  palau_bsh_proj$DOA <- mean(palau_bsh_total_DOA)
  palau_bsh_proj$DOA.upr <- quantile(palau_bsh_total_DOA, 0.95)
  palau_bsh_proj$DOA.lwr <- quantile(palau_bsh_total_DOA, 0.05)
  
  palau_bsh_total_PRM <- colSums(palau_bsh_PRM)
  palau_bsh_proj$PRM <- mean(palau_bsh_total_PRM)
  palau_bsh_proj$PRM.upr <- quantile(palau_bsh_total_PRM, 0.95)
  palau_bsh_proj$PRM.lwr <- quantile(palau_bsh_total_PRM, 0.05)
  
  palau_bsh_total_total <- colSums(palau_bsh_total)
  palau_bsh_proj$total <- mean(palau_bsh_total_total)
  palau_bsh_proj$total.upr <- quantile(palau_bsh_total_total, 0.95)
  palau_bsh_proj$total.lwr <- quantile(palau_bsh_total_total, 0.05)
  ...
  palau_silky_proj$catch.upr <- quantile(palau_silky_total_catches, 0.95)
  palau_silky_proj$catch.lwr <- quantile(palau_silky_total_catches, 0.05)
  
  palau_silky_total_DOA <- colSums(palau_silky_DOA)
  palau_silky_proj$DOA <- mean(palau_silky_total_DOA)
  palau_silky_proj$DOA.upr <- quantile(palau_silky_total_DOA, 0.95)
  palau_silky_proj$DOA.lwr <- quantile(palau_silky_total_DOA, 0.05)
  
  palau_silky_total_PRM <- colSums(palau_silky_PRM)
  palau_silky_proj$PRM <- mean(palau_silky_total_PRM)
  palau_silky_proj$PRM.upr <- quantile(palau_silky_total_PRM, 0.95)
  palau_silky_proj$PRM.lwr <- quantile(palau_silky_total_PRM, 0.05)
  
  palau_silky_total_total <- colSums(palau_silky_total)
  palau_silky_proj$total <- mean(palau_silky_total_total)
  palau_silky_proj$total.upr <- quantile(palau_silky_total_total, 0.95)
  palau_silky_proj$total.lwr <- quantile(palau_silky_total_total, 0.05)
  ...
  palau_thresher_proj$catch.upr <- quantile(palau_thresher_total_catches, 0.95)
  palau_thresher_proj$catch.lwr <- quantile(palau_thresher_total_catches, 0.05)
  
  palau_thresher_total_DOA <- colSums(palau_thresher_DOA)
  palau_thresher_proj$DOA <- mean(palau_thresher_total_DOA)
  palau_thresher_proj$DOA.upr <- quantile(palau_thresher_total_DOA, 0.95)
  palau_thresher_proj$DOA.lwr <- quantile(palau_thresher_total_DOA, 0.05)
  
  palau_thresher_total_PRM <- colSums(palau_thresher_PRM)
  palau_thresher_proj$PRM <- mean(palau_thresher_total_PRM)
  palau_thresher_proj$PRM.upr <- quantile(palau_thresher_total_PRM, 0.95)
  palau_thresher_proj$PRM.lwr <- quantile(palau_thresher_total_PRM, 0.05)
  
  palau_thresher_total_total <- colSums(palau_thresher_total)
  palau_thresher_proj$total <- mean(palau_thresher_total_total)
  palau_thresher_proj$total.upr <- quantile(palau_thresher_total_total, 0.95)
  palau_thresher_proj$total.lwr <- quantile(palau_thresher_total_total, 0.05)
  ...
  #run averages
  palau_mako_proj <- data.frame("Species" = "Mako Shark",
                                     "catch" = NA,
                                     "catch.upr" = NA,
                                     "catch.lwr" = NA,
                                     "DOA" = NA,
  ...
  palau_mako_proj$catch.upr <- quantile(palau_mako_total_catches, 0.95)
  palau_mako_proj$catch.lwr <- quantile(palau_mako_total_catches, 0.05)
  
  palau_mako_total_DOA <- colSums(palau_mako_DOA)
  palau_mako_proj$DOA <- mean(palau_mako_total_DOA)
  palau_mako_proj$DOA.upr <- quantile(palau_mako_total_DOA, 0.95)
  palau_mako_proj$DOA.lwr <- quantile(palau_mako_total_DOA, 0.05)
  
  palau_mako_total_PRM <- colSums(palau_mako_PRM)
  palau_mako_proj$PRM <- mean(palau_mako_total_PRM)
  palau_mako_proj$PRM.upr <- quantile(palau_mako_total_PRM, 0.95)
  palau_mako_proj$PRM.lwr <- quantile(palau_mako_total_PRM, 0.05)
  
  palau_mako_total_total <- colSums(palau_mako_total)
  palau_mako_proj$total <- mean(palau_mako_total_total)
  palau_mako_proj$total.upr <- quantile(palau_mako_total_total, 0.95)
  palau_mako_proj$total.lwr <- quantile(palau_mako_total_total, 0.05)
  ...
  palau_oceanicwhitetip_proj$catch.upr <- quantile(palau_oceanicwhitetip_total_catches, 0.95)
  palau_oceanicwhitetip_proj$catch.lwr <- quantile(palau_oceanicwhitetip_total_catches, 0.05)
  ...
  palau_oceanicwhitetip_proj$DOA.upr <- quantile(palau_oceanicwhitetip_total_DOA, 0.95)
  palau_oceanicwhitetip_proj$DOA.lwr <- quantile(palau_oceanicwhitetip_total_DOA, 0.05)
  ...
  palau_oceanicwhitetip_proj$PRM.upr <- quantile(palau_oceanicwhitetip_total_PRM, 0.95)
  palau_oceanicwhitetip_proj$PRM.lwr <- quantile(palau_oceanicwhitetip_total_PRM, 0.05)
  ...
  palau_oceanicwhitetip_proj$total.upr <- quantile(palau_oceanicwhitetip_total_total, 0.95)
  palau_oceanicwhitetip_proj$total.lwr <- quantile(palau_oceanicwhitetip_total_total, 0.05)
  ...
  palau_hammerhead_proj$catch.upr <- quantile(palau_hammerhead_total_catches, 0.95)
  palau_hammerhead_proj$catch.lwr <- quantile(palau_hammerhead_total_catches, 0.05)
  
  palau_hammerhead_total_DOA <- colSums(palau_hammerhead_DOA)
  palau_hammerhead_proj$DOA <- mean(palau_hammerhead_total_DOA)
  palau_hammerhead_proj$DOA.upr <- quantile(palau_hammerhead_total_DOA, 0.95)
  palau_hammerhead_proj$DOA.lwr <- quantile(palau_hammerhead_total_DOA, 0.05)
  
  palau_hammerhead_total_PRM <- colSums(palau_hammerhead_PRM)
  palau_hammerhead_proj$PRM <- mean(palau_hammerhead_total_PRM)
  palau_hammerhead_proj$PRM.upr <- quantile(palau_hammerhead_total_PRM, 0.95)
  palau_hammerhead_proj$PRM.lwr <- quantile(palau_hammerhead_total_PRM, 0.05)
  
  palau_hammerhead_total_total <- colSums(palau_hammerhead_total)
  palau_hammerhead_proj$total <- mean(palau_hammerhead_total_total)
  palau_hammerhead_proj$total.upr <- quantile(palau_hammerhead_total_total, 0.95)
  palau_hammerhead_proj$total.lwr <- quantile(palau_hammerhead_total_total, 0.05)
  ...
  palau_othersharks_proj$catch.upr <- quantile(palau_othersharks_total_catches, 0.95)
  palau_othersharks_proj$catch.lwr <- quantile(palau_othersharks_total_catches, 0.05)
  
  palau_othersharks_total_DOA <- colSums(palau_othersharks_DOA)
  palau_othersharks_proj$DOA <- mean(palau_othersharks_total_DOA)
  palau_othersharks_proj$DOA.upr <- quantile(palau_othersharks_total_DOA, 0.95)
  palau_othersharks_proj$DOA.lwr <- quantile(palau_othersharks_total_DOA, 0.05)
  
  palau_othersharks_total_PRM <- colSums(palau_othersharks_PRM)
  palau_othersharks_proj$PRM <- mean(palau_othersharks_total_PRM)
  palau_othersharks_proj$PRM.upr <- quantile(palau_othersharks_total_PRM, 0.95)
  palau_othersharks_proj$PRM.lwr <- quantile(palau_othersharks_total_PRM, 0.05)
  ...
  palau_othersharks_proj$total.upr <- quantile(palau_othersharks_total_total, 0.95)
  palau_othersharks_proj$total.lwr <- quantile(palau_othersharks_total_total, 0.05)
