library(tidyverse)
library(fields)
library(stringr)
library(sf)
library(logitnorm)


set.seed(1000)

rm(list=ls())
setwd('~/Desktop/Ongoing Projects/GFW Shark Sanctuary/working_dir')

#CPUE data
master_cpue <- read.csv("~/Desktop/Ongoing Projects/GFW Shark Sanctuary/working_dir/WCPFC CPUEs.csv")
master_cpue <- master_cpue[,-4] #get rid of Log_hooks column so we can use model estiamted log_hooks
master_cpue$Long <- ifelse(master_cpue$Long2 >= 180, master_cpue$Long2 - 360, master_cpue$Long2) #correct back for lat/lon

#### mortality ####
mortalityrates <-read.csv("mortalityrates2.csv")

#mins and maxes for uniform distributions for sample and replace
mortalityrates$HookMin <- with(mortalityrates, pmin(Hook_mortality_gilman, Hook_mortality_hutch, na.rm=T))
mortalityrates$HookMax <- with(mortalityrates, pmax(Hook_mortality_gilman, Hook_mortality_hutch, na.rm=T))

mortalityrates$logit.prm<-log(mortalityrates$prm.mean/(1-mortalityrates$prm.mean))
mortalityrates$logit.prm.upr <- log(mortalityrates$prm.upr/(1-mortalityrates$prm.upr))
mortalityrates$logit.prm.lwr <- log(mortalityrates$prm.lwr/(1-mortalityrates$prm.lwr))
mortalityrates$diff1 <- mortalityrates$logit.prm.upr-mortalityrates$logit.prm
mortalityrates$diff2 <- mortalityrates$logit.prm-mortalityrates$logit.prm.lwr
mortalityrates$logit.prm.se <- rowMeans(mortalityrates[c(13:14)])

mortality <- mortalityrates[,c(1,2,8:10,15)]

#### Hook data ####
micronesia_proj <- read.csv("micronesia2019.csv")

micronesia_proj2 <- micronesia_proj[,c(5,6,9,10)]
#rename
colnames(micronesia_proj2)[1] <- "Long"
colnames(micronesia_proj2)[2] <- "Lat"

#merge
micronesia_comb <- merge(micronesia_proj2, master_cpue, by=c("Lat", "Long"), all=T)
micronesia_comb <- micronesia_comb %>% drop_na(log_hooks)

n <- nrow(micronesia_comb)

micronesia_hook_preds <- matrix(nrow=n, ncol=1000)
for (j in 1:n) {
  for (i in 1:1000) {
    micronesia_hook_preds[j,i] <- rlnorm(1,micronesia_comb$log_hooks[j],micronesia_comb$log_hooks.se[j])
  }
}

{start.time <- Sys.time()
  #### blue sharks ####
  #row 2 in mortality; need to change this for each species when drawing rates)
  r_bsh = 2
  
  micronesia_bsh <- micronesia_comb[,c(1:4,9,10)]
  
  micronesia_bsh_catches <- matrix(nrow=n, ncol=1000)
  micronesia_bsh_DOA <- matrix(nrow=n, ncol=1000)
  micronesia_bsh_released <- matrix(nrow=n, ncol=1000)
  micronesia_bsh_PRM<- matrix(nrow=n, ncol=1000)
  micronesia_bsh_total <- matrix(nrow=n, ncol=1000)
  
  micronesia_bsh_CM_rate <- runif(1000,mortality$HookMin[r_bsh],mortality$HookMax[r_bsh])
  micronesia_bsh_PRM_rate <- rlogitnorm(1000, mortality$logit.prm[r_bsh],mortality$logit.prm.se[r_bsh])
  
  for (j in 1:n) {
    for (i in 1:1000) {
      micronesia_bsh_catches[j,i] <- micronesia_hook_preds[j,i]/1000 * rlnorm(1, micronesia_comb$blueshark.logcpue[j], micronesia_comb$blueshark.logcpue.se[j])
      micronesia_bsh_DOA[j,i] <- micronesia_bsh_catches[j,i] *  micronesia_bsh_CM_rate[i]
      micronesia_bsh_released[j,i] <- micronesia_bsh_catches[j,i] - micronesia_bsh_DOA[j,i]
      micronesia_bsh_PRM[j,i] <- micronesia_bsh_released[j,i] * micronesia_bsh_PRM_rate[i] 
      micronesia_bsh_total[j,i] <- micronesia_bsh_DOA[j,i] + micronesia_bsh_PRM[j,i]
    }
  }
  
  #run averages
  micronesia_bsh_proj <- data.frame("Species" = "Blue Shark",
                               "catch" = NA,
                               "catch.upr" = NA,
                               "catch.lwr" = NA,
                               "DOA" = NA,
                               "DOA.upr" = NA,
                               "DOA.lwr" = NA,
                               "PRM" = NA,
                               "PRM.upr" = NA,
                               "PRM.lwr" = NA,
                               "total" = NA,
                               "total.upr" = NA,
                               "total.lwr" = NA)
  
  micronesia_bsh_total_catches <- colSums(micronesia_bsh_catches)
  micronesia_bsh_proj$catch <- mean(micronesia_bsh_total_catches)
  micronesia_bsh_proj$catch.upr <- quantile(micronesia_bsh_total_catches, 0.95)
  micronesia_bsh_proj$catch.lwr <- quantile(micronesia_bsh_total_catches, 0.05)
  
  micronesia_bsh_total_DOA <- colSums(micronesia_bsh_DOA)
  micronesia_bsh_proj$DOA <- mean(micronesia_bsh_total_DOA)
  micronesia_bsh_proj$DOA.upr <- quantile(micronesia_bsh_total_DOA, 0.95)
  micronesia_bsh_proj$DOA.lwr <- quantile(micronesia_bsh_total_DOA, 0.05)
  
  micronesia_bsh_total_PRM <- colSums(micronesia_bsh_PRM)
  micronesia_bsh_proj$PRM <- mean(micronesia_bsh_total_PRM)
  micronesia_bsh_proj$PRM.upr <- quantile(micronesia_bsh_total_PRM, 0.95)
  micronesia_bsh_proj$PRM.lwr <- quantile(micronesia_bsh_total_PRM, 0.05)
  
  micronesia_bsh_total_total <- colSums(micronesia_bsh_total)
  micronesia_bsh_proj$total <- mean(micronesia_bsh_total_total)
  micronesia_bsh_proj$total.upr <- quantile(micronesia_bsh_total_total, 0.95)
  micronesia_bsh_proj$total.lwr <- quantile(micronesia_bsh_total_total, 0.05)
  
  #cell averages
  #catches
  micronesia_bsh$catch <- rowMeans(micronesia_bsh_catches[,1:1000])
  micronesia_bsh$catch.sd = apply(micronesia_bsh_catches[,1:1000], 1, sd)
  
  #DOA
  micronesia_bsh$DOA <- rowMeans(micronesia_bsh_DOA[,1:1000])
  micronesia_bsh$DOA.sd = apply(micronesia_bsh_DOA[,1:1000], 1, sd)
  
  #prm
  micronesia_bsh$PRM = rowMeans(micronesia_bsh_PRM[,1:1000])
  micronesia_bsh$PRM.sd = apply(micronesia_bsh_PRM[,1:1000], 1, sd)
  
  #total
  micronesia_bsh$total = rowMeans(micronesia_bsh_total[,1:1000])
  micronesia_bsh$total.sd = apply(micronesia_bsh_total[,1:1000], 1, sd)
  
  #### silky sharks ####
  r_silky=5 #row 5 in mortality; need to change this for each species when drawing rates)
  
  micronesia_silky <- micronesia_comb[,c(1:4,13,14)] #create dataframe
  
  micronesia_silky_catches <- matrix(nrow=n, ncol=1000)
  micronesia_silky_DOA <- matrix(nrow=n, ncol=1000)
  micronesia_silky_released <- matrix(nrow=n, ncol=1000)
  micronesia_silky_PRM<- matrix(nrow=n, ncol=1000)
  micronesia_silky_total <- matrix(nrow=n, ncol=1000)
  
  micronesia_silky_CM_rate <- runif(1000,mortality$HookMin[r_silky],mortality$HookMax[r_silky])
  micronesia_silky_PRM_rate <- rlogitnorm(1000, mortality$logit.prm[r_silky],mortality$logit.prm.se[r_silky])
  
  
  for (j in 1:n) {
    for (i in 1:1000) {
      micronesia_silky_catches[j,i] <- micronesia_hook_preds[j,i]/1000 * rlnorm(1, micronesia_comb$silkyshark.logcpue[j], micronesia_comb$silkyshark.logcpue.se[j])
      micronesia_silky_DOA[j,i] <- micronesia_silky_catches[j,i] * micronesia_silky_CM_rate[i]
      micronesia_silky_released[j,i] <- micronesia_silky_catches[j,i] - micronesia_silky_DOA[j,i]
      micronesia_silky_PRM[j,i] <- micronesia_silky_released[j,i] * micronesia_silky_PRM_rate[i]
      micronesia_silky_total[j,i] <- micronesia_silky_DOA[j,i] + micronesia_silky_PRM[j,i]
    }
  }
  micronesia_silky_proj <- data.frame("Species" = "Silky Shark",
                                    "catch" = NA,
                                    "catch.upr" = NA,
                                    "catch.lwr" = NA,
                                    "DOA" = NA,
                                    "DOA.upr" = NA,
                                    "DOA.lwr" = NA,
                                    "PRM" = NA,
                                    "PRM.upr" = NA,
                                    "PRM.lwr" = NA,
                                    "total" = NA,
                                    "total.upr" = NA,
                                    "total.lwr" = NA)
  
  micronesia_silky_total_catches <- colSums(micronesia_silky_catches)
  micronesia_silky_proj$catch <- mean(micronesia_silky_total_catches)
  micronesia_silky_proj$catch.upr <- quantile(micronesia_silky_total_catches, 0.95)
  micronesia_silky_proj$catch.lwr <- quantile(micronesia_silky_total_catches, 0.05)
  
  micronesia_silky_total_DOA <- colSums(micronesia_silky_DOA)
  micronesia_silky_proj$DOA <- mean(micronesia_silky_total_DOA)
  micronesia_silky_proj$DOA.upr <- quantile(micronesia_silky_total_DOA, 0.95)
  micronesia_silky_proj$DOA.lwr <- quantile(micronesia_silky_total_DOA, 0.05)
  
  micronesia_silky_total_PRM <- colSums(micronesia_silky_PRM)
  micronesia_silky_proj$PRM <- mean(micronesia_silky_total_PRM)
  micronesia_silky_proj$PRM.upr <- quantile(micronesia_silky_total_PRM, 0.95)
  micronesia_silky_proj$PRM.lwr <- quantile(micronesia_silky_total_PRM, 0.05)
  
  micronesia_silky_total_total <- colSums(micronesia_silky_total)
  micronesia_silky_proj$total <- mean(micronesia_silky_total_total)
  micronesia_silky_proj$total.upr <- quantile(micronesia_silky_total_total, 0.95)
  micronesia_silky_proj$total.lwr <- quantile(micronesia_silky_total_total, 0.05)
  
  #cell averages
  #catches
  micronesia_silky$catch <- rowMeans(micronesia_silky_catches[,1:1000])
  micronesia_silky$catch.sd = apply(micronesia_silky_catches[,1:1000], 1, sd)
  
  #DOA
  micronesia_silky$DOA <- rowMeans(micronesia_silky_DOA[,1:1000])
  micronesia_silky$DOA.sd = apply(micronesia_silky_DOA[,1:1000], 1, sd)
  
  #prm
  micronesia_silky$PRM = rowMeans(micronesia_silky_PRM[,1:1000])
  micronesia_silky$PRM.sd = apply(micronesia_silky_PRM[,1:1000], 1, sd)
  
  #total
  micronesia_silky$total = rowMeans(micronesia_silky_total[,1:1000])
  micronesia_silky$total.sd = apply(micronesia_silky_total[,1:1000], 1, sd)
  
  #### thresher sharks ####
  #row 1 in mortality; need to change this for each species when drawing rates)
  r_thresher = 1
  
  micronesia_thresher <- micronesia_comb[,c(1:4,21,22)] #create dataframe
  
  micronesia_thresher_catches <- matrix(nrow=n, ncol=1000)
  micronesia_thresher_DOA <- matrix(nrow=n, ncol=1000)
  micronesia_thresher_released <- matrix(nrow=n, ncol=1000)
  micronesia_thresher_PRM<- matrix(nrow=n, ncol=1000)
  micronesia_thresher_total <- matrix(nrow=n, ncol=1000)
  
  micronesia_thresher_CM_rate <- runif(1000,mortality$HookMin[r_thresher],mortality$HookMax[r_thresher])
  micronesia_thresher_PRM_rate <- rlogitnorm(1000, mortality$logit.prm[r_thresher],mortality$logit.prm.se[r_thresher])
  
  for (j in 1:n) {
    for (i in 1:1000) {
      micronesia_thresher_catches[j,i] <- micronesia_hook_preds[j,i]/1000 * rlnorm(1, micronesia_comb$thresher.logcpue[j], micronesia_comb$thresher.logcpue.se[j])
      micronesia_thresher_DOA[j,i] <- micronesia_thresher_catches[j,i] * micronesia_thresher_CM_rate[i]
      micronesia_thresher_released[j,i] <- micronesia_thresher_catches[j,i] - micronesia_thresher_DOA[j,i]
      micronesia_thresher_PRM[j,i] <- micronesia_thresher_released[j,i] * micronesia_thresher_PRM_rate[i]
      micronesia_thresher_total[j,i] <- micronesia_thresher_DOA[j,i] + micronesia_thresher_PRM[j,i]
    }
  }
  
  micronesia_thresher_proj <- data.frame("Species" = "Thresher",
                                    "catch" = NA,
                                    "catch.upr" = NA,
                                    "catch.lwr" = NA,
                                    "DOA" = NA,
                                    "DOA.upr" = NA,
                                    "DOA.lwr" = NA,
                                    "PRM" = NA,
                                    "PRM.upr" = NA,
                                    "PRM.lwr" = NA,
                                    "total" = NA,
                                    "total.upr" = NA,
                                    "total.lwr" = NA)
  
  micronesia_thresher_total_catches <- colSums(micronesia_thresher_catches)
  micronesia_thresher_proj$catch <- mean(micronesia_thresher_total_catches)
  micronesia_thresher_proj$catch.upr <- quantile(micronesia_thresher_total_catches, 0.95)
  micronesia_thresher_proj$catch.lwr <- quantile(micronesia_thresher_total_catches, 0.05)
  
  micronesia_thresher_total_DOA <- colSums(micronesia_thresher_DOA)
  micronesia_thresher_proj$DOA <- mean(micronesia_thresher_total_DOA)
  micronesia_thresher_proj$DOA.upr <- quantile(micronesia_thresher_total_DOA, 0.95)
  micronesia_thresher_proj$DOA.lwr <- quantile(micronesia_thresher_total_DOA, 0.05)
  
  micronesia_thresher_total_PRM <- colSums(micronesia_thresher_PRM)
  micronesia_thresher_proj$PRM <- mean(micronesia_thresher_total_PRM)
  micronesia_thresher_proj$PRM.upr <- quantile(micronesia_thresher_total_PRM, 0.95)
  micronesia_thresher_proj$PRM.lwr <- quantile(micronesia_thresher_total_PRM, 0.05)
  
  micronesia_thresher_total_total <- colSums(micronesia_thresher_total)
  micronesia_thresher_proj$total <- mean(micronesia_thresher_total_total)
  micronesia_thresher_proj$total.upr <- quantile(micronesia_thresher_total_total, 0.95)
  micronesia_thresher_proj$total.lwr <- quantile(micronesia_thresher_total_total, 0.05)
  
  #cell averages
  #catches
  micronesia_thresher$catch <- rowMeans(micronesia_thresher_catches[,1:1000])
  micronesia_thresher$catch.sd = apply(micronesia_thresher_catches[,1:1000], 1, sd)
  
  #DOA
  micronesia_thresher$DOA <- rowMeans(micronesia_thresher_DOA[,1:1000])
  micronesia_thresher$DOA.sd = apply(micronesia_thresher_DOA[,1:1000], 1, sd)
  
  #prm
  micronesia_thresher$PRM = rowMeans(micronesia_thresher_PRM[,1:1000])
  micronesia_thresher$PRM.sd = apply(micronesia_thresher_PRM[,1:1000], 1, sd)
  
  #total
  micronesia_thresher$total = rowMeans(micronesia_thresher_total[,1:1000])
  micronesia_thresher$total.sd = apply(micronesia_thresher_total[,1:1000], 1, sd)
  
  #### shortfinmako sharks ####
  #row 4 in mortality; need to change this for each species when drawing rates)
  r_mako = 4
  
  micronesia_mako <- micronesia_comb[,c(1:4,17,18)] #create dataframe
  
  micronesia_mako_catches <- matrix(nrow=n, ncol=1000)
  micronesia_mako_DOA <- matrix(nrow=n, ncol=1000)
  micronesia_mako_released <- matrix(nrow=n, ncol=1000)
  micronesia_mako_PRM<- matrix(nrow=n, ncol=1000)
  micronesia_mako_total <- matrix(nrow=n, ncol=1000)
  
  micronesia_mako_CM_rate <- runif(1000,mortality$HookMin[r_mako],mortality$HookMax[r_mako])
  micronesia_mako_PRM_rate <- rlogitnorm(1000, mortality$logit.prm[r_mako],mortality$logit.prm.se[r_mako])
  
  
  for (j in 1:n) {
    for (i in 1:1000) {
      micronesia_mako_catches[j,i] <- micronesia_hook_preds[j,i]/1000 * rlnorm(1, micronesia_comb$mako.logcpue[j], micronesia_comb$mako.logcpue.se[j])
      micronesia_mako_DOA[j,i] <- micronesia_mako_catches[j,i] * micronesia_mako_CM_rate[i]
      micronesia_mako_released[j,i] <- micronesia_mako_catches[j,i] - micronesia_mako_DOA[j,i]
      micronesia_mako_PRM[j,i] <- micronesia_mako_released[j,i] * micronesia_mako_PRM_rate[i]
      micronesia_mako_total[j,i] <- micronesia_mako_DOA[j,i] + micronesia_mako_PRM[j,i]
    }
  }
  
  #run averages
  micronesia_mako_proj <- data.frame("Species" = "Mako Shark",
                                    "catch" = NA,
                                    "catch.upr" = NA,
                                    "catch.lwr" = NA,
                                    "DOA" = NA,
                                    "DOA.upr" = NA,
                                    "DOA.lwr" = NA,
                                    "PRM" = NA,
                                    "PRM.upr" = NA,
                                    "PRM.lwr" = NA,
                                    "total" = NA,
                                    "total.upr" = NA,
                                    "total.lwr" = NA)
  
  micronesia_mako_total_catches <- colSums(micronesia_mako_catches)
  micronesia_mako_proj$catch <- mean(micronesia_mako_total_catches)
  micronesia_mako_proj$catch.upr <- quantile(micronesia_mako_total_catches, 0.95)
  micronesia_mako_proj$catch.lwr <- quantile(micronesia_mako_total_catches, 0.05)
  
  micronesia_mako_total_DOA <- colSums(micronesia_mako_DOA)
  micronesia_mako_proj$DOA <- mean(micronesia_mako_total_DOA)
  micronesia_mako_proj$DOA.upr <- quantile(micronesia_mako_total_DOA, 0.95)
  micronesia_mako_proj$DOA.lwr <- quantile(micronesia_mako_total_DOA, 0.05)
  
  micronesia_mako_total_PRM <- colSums(micronesia_mako_PRM)
  micronesia_mako_proj$PRM <- mean(micronesia_mako_total_PRM)
  micronesia_mako_proj$PRM.upr <- quantile(micronesia_mako_total_PRM, 0.95)
  micronesia_mako_proj$PRM.lwr <- quantile(micronesia_mako_total_PRM, 0.05)
  
  micronesia_mako_total_total <- colSums(micronesia_mako_total)
  micronesia_mako_proj$total <- mean(micronesia_mako_total_total)
  micronesia_mako_proj$total.upr <- quantile(micronesia_mako_total_total, 0.95)
  micronesia_mako_proj$total.lwr <- quantile(micronesia_mako_total_total, 0.05)
  
  #cell averages
  #catches
  micronesia_mako$catch <- rowMeans(micronesia_mako_catches[,1:1000])
  micronesia_mako$catch.sd = apply(micronesia_mako_catches[,1:1000], 1, sd)
  
  #DOA
  micronesia_mako$DOA <- rowMeans(micronesia_mako_DOA[,1:1000])
  micronesia_mako$DOA.sd = apply(micronesia_mako_DOA[,1:1000], 1, sd)
  
  #prm
  micronesia_mako$PRM = rowMeans(micronesia_mako_PRM[,1:1000])
  micronesia_mako$PRM.sd = apply(micronesia_mako_PRM[,1:1000], 1, sd)
  
  #total
  micronesia_mako$total = rowMeans(micronesia_mako_total[,1:1000])
  micronesia_mako$total.sd = apply(micronesia_mako_total[,1:1000], 1, sd)
  
  #### oceanic whitetip sharks ####
  #row 8 in mortality; need to change this for each species when drawing rates)
  r_oceanicwhitetip = 8
  
  micronesia_oceanicwhitetip <- micronesia_comb[,c(1:4,25,26)] #create dataframe
  
  micronesia_oceanicwhitetip_catches <- matrix(nrow=n, ncol=1000)
  micronesia_oceanicwhitetip_DOA <- matrix(nrow=n, ncol=1000)
  micronesia_oceanicwhitetip_released <- matrix(nrow=n, ncol=1000)
  micronesia_oceanicwhitetip_PRM<- matrix(nrow=n, ncol=1000)
  micronesia_oceanicwhitetip_total <- matrix(nrow=n, ncol=1000)
  
  micronesia_oceanicwhitetip_CM_rate <- runif(1000,mortality$HookMin[r_oceanicwhitetip],mortality$HookMax[r_oceanicwhitetip])
  micronesia_oceanicwhitetip_PRM_rate <- rlogitnorm(1000, mortality$logit.prm[r_oceanicwhitetip],mortality$logit.prm.se[r_oceanicwhitetip])
  
  for (j in 1:n) {
    for (i in 1:1000) {
      micronesia_oceanicwhitetip_catches[j,i] <- micronesia_hook_preds[j,i]/1000 * rlnorm(1, micronesia_comb$oceanicwhitetip.logcpue[j], micronesia_comb$oceanicwhitetip.logcpue.se[j])
      micronesia_oceanicwhitetip_DOA[j,i] <- micronesia_oceanicwhitetip_catches[j,i] * micronesia_oceanicwhitetip_CM_rate[i]
      micronesia_oceanicwhitetip_released[j,i] <- micronesia_oceanicwhitetip_catches[j,i] - micronesia_oceanicwhitetip_DOA[j,i]
      micronesia_oceanicwhitetip_PRM[j,i] <- micronesia_oceanicwhitetip_released[j,i] * micronesia_oceanicwhitetip_PRM_rate[i]
      micronesia_oceanicwhitetip_total[j,i] <- micronesia_oceanicwhitetip_DOA[j,i] + micronesia_oceanicwhitetip_PRM[j,i]
    }
  }
  
  #run averages
  micronesia_oceanicwhitetip_proj <- data.frame("Species" = "Oceanic Whitetip",
                                    "catch" = NA,
                                    "catch.upr" = NA,
                                    "catch.lwr" = NA,
                                    "DOA" = NA,
                                    "DOA.upr" = NA,
                                    "DOA.lwr" = NA,
                                    "PRM" = NA,
                                    "PRM.upr" = NA,
                                    "PRM.lwr" = NA,
                                    "total" = NA,
                                    "total.upr" = NA,
                                    "total.lwr" = NA)
  
  micronesia_oceanicwhitetip_total_catches <- colSums(micronesia_oceanicwhitetip_catches)
  micronesia_oceanicwhitetip_proj$catch <- mean(micronesia_oceanicwhitetip_total_catches)
  micronesia_oceanicwhitetip_proj$catch.upr <- quantile(micronesia_oceanicwhitetip_total_catches, 0.95)
  micronesia_oceanicwhitetip_proj$catch.lwr <- quantile(micronesia_oceanicwhitetip_total_catches, 0.05)
  
  micronesia_oceanicwhitetip_total_DOA <- colSums(micronesia_oceanicwhitetip_DOA)
  micronesia_oceanicwhitetip_proj$DOA <- mean(micronesia_oceanicwhitetip_total_DOA)
  micronesia_oceanicwhitetip_proj$DOA.upr <- quantile(micronesia_oceanicwhitetip_total_DOA, 0.95)
  micronesia_oceanicwhitetip_proj$DOA.lwr <- quantile(micronesia_oceanicwhitetip_total_DOA, 0.05)
  
  micronesia_oceanicwhitetip_total_PRM <- colSums(micronesia_oceanicwhitetip_PRM)
  micronesia_oceanicwhitetip_proj$PRM <- mean(micronesia_oceanicwhitetip_total_PRM)
  micronesia_oceanicwhitetip_proj$PRM.upr <- quantile(micronesia_oceanicwhitetip_total_PRM, 0.95)
  micronesia_oceanicwhitetip_proj$PRM.lwr <- quantile(micronesia_oceanicwhitetip_total_PRM, 0.05)
  
  micronesia_oceanicwhitetip_total_total <- colSums(micronesia_oceanicwhitetip_total)
  micronesia_oceanicwhitetip_proj$total <- mean(micronesia_oceanicwhitetip_total_total)
  micronesia_oceanicwhitetip_proj$total.upr <- quantile(micronesia_oceanicwhitetip_total_total, 0.95)
  micronesia_oceanicwhitetip_proj$total.lwr <- quantile(micronesia_oceanicwhitetip_total_total, 0.05)
  
  #cell averages
  #catches
  micronesia_oceanicwhitetip$catch <- rowMeans(micronesia_oceanicwhitetip_catches[,1:1000])
  micronesia_oceanicwhitetip$catch.sd = apply(micronesia_oceanicwhitetip_catches[,1:1000], 1, sd)
  
  #DOA
  micronesia_oceanicwhitetip$DOA <- rowMeans(micronesia_oceanicwhitetip_DOA[,1:1000])
  micronesia_oceanicwhitetip$DOA.sd = apply(micronesia_oceanicwhitetip_DOA[,1:1000], 1, sd)
  
  #prm
  micronesia_oceanicwhitetip$PRM = rowMeans(micronesia_oceanicwhitetip_PRM[,1:1000])
  micronesia_oceanicwhitetip$PRM.sd = apply(micronesia_oceanicwhitetip_PRM[,1:1000], 1, sd)
  
  #total
  micronesia_oceanicwhitetip$total = rowMeans(micronesia_oceanicwhitetip_total[,1:1000])
  micronesia_oceanicwhitetip$total.sd = apply(micronesia_oceanicwhitetip_total[,1:1000], 1, sd)
  
  #### hammerhead sharks ####
  #row 9 in mortality; need to change this for each species when drawing rates)
  r_hammerhead = 9
  
  micronesia_hammerhead <- micronesia_comb[,c(1:4,29,30)] #create dataframe
  
  micronesia_hammerhead_catches <- matrix(nrow=n, ncol=1000)
  micronesia_hammerhead_DOA <- matrix(nrow=n, ncol=1000)
  micronesia_hammerhead_released <- matrix(nrow=n, ncol=1000)
  micronesia_hammerhead_PRM<- matrix(nrow=n, ncol=1000)
  micronesia_hammerhead_total <- matrix(nrow=n, ncol=1000)
  
  micronesia_hammerhead_CM_rate <- runif(1000,mortality$HookMin[r_hammerhead],mortality$HookMax[r_hammerhead])
  micronesia_hammerhead_PRM_rate <- rlogitnorm(1000, mortality$logit.prm[r_hammerhead],mortality$logit.prm.se[r_hammerhead])
  
  for (j in 1:n) {
    for (i in 1:1000) {
      micronesia_hammerhead_catches[j,i] <- micronesia_hook_preds[j,i]/1000 * rlnorm(1, micronesia_comb$hammerhead.logcpue[j], micronesia_comb$hammerhead.logcpue.se[j])
      micronesia_hammerhead_DOA[j,i] <- micronesia_hammerhead_catches[j,i] * micronesia_hammerhead_CM_rate[i]
      micronesia_hammerhead_released[j,i] <- micronesia_hammerhead_catches[j,i] - micronesia_hammerhead_DOA[j,i]
      micronesia_hammerhead_PRM[j,i] <- micronesia_hammerhead_released[j,i] * micronesia_hammerhead_PRM_rate[i]
      micronesia_hammerhead_total[j,i] <- micronesia_hammerhead_DOA[j,i] + micronesia_hammerhead_PRM[j,i]
    }
  }
  
  #run averages
  micronesia_hammerhead_proj <- data.frame("Species" = "Hammerhead",
                                    "catch" = NA,
                                    "catch.upr" = NA,
                                    "catch.lwr" = NA,
                                    "DOA" = NA,
                                    "DOA.upr" = NA,
                                    "DOA.lwr" = NA,
                                    "PRM" = NA,
                                    "PRM.upr" = NA,
                                    "PRM.lwr" = NA,
                                    "total" = NA,
                                    "total.upr" = NA,
                                    "total.lwr" = NA)
  
  micronesia_hammerhead_total_catches <- colSums(micronesia_hammerhead_catches)
  micronesia_hammerhead_proj$catch <- mean(micronesia_hammerhead_total_catches)
  micronesia_hammerhead_proj$catch.upr <- quantile(micronesia_hammerhead_total_catches, 0.95)
  micronesia_hammerhead_proj$catch.lwr <- quantile(micronesia_hammerhead_total_catches, 0.05)
  
  micronesia_hammerhead_total_DOA <- colSums(micronesia_hammerhead_DOA)
  micronesia_hammerhead_proj$DOA <- mean(micronesia_hammerhead_total_DOA)
  micronesia_hammerhead_proj$DOA.upr <- quantile(micronesia_hammerhead_total_DOA, 0.95)
  micronesia_hammerhead_proj$DOA.lwr <- quantile(micronesia_hammerhead_total_DOA, 0.05)
  
  micronesia_hammerhead_total_PRM <- colSums(micronesia_hammerhead_PRM)
  micronesia_hammerhead_proj$PRM <- mean(micronesia_hammerhead_total_PRM)
  micronesia_hammerhead_proj$PRM.upr <- quantile(micronesia_hammerhead_total_PRM, 0.95)
  micronesia_hammerhead_proj$PRM.lwr <- quantile(micronesia_hammerhead_total_PRM, 0.05)
  
  micronesia_hammerhead_total_total <- colSums(micronesia_hammerhead_total)
  micronesia_hammerhead_proj$total <- mean(micronesia_hammerhead_total_total)
  micronesia_hammerhead_proj$total.upr <- quantile(micronesia_hammerhead_total_total, 0.95)
  micronesia_hammerhead_proj$total.lwr <- quantile(micronesia_hammerhead_total_total, 0.05)
  
  #cell averages
  #catches
  micronesia_hammerhead$catch <- rowMeans(micronesia_hammerhead_catches[,1:1000])
  micronesia_hammerhead$catch.sd = apply(micronesia_hammerhead_catches[,1:1000], 1, sd)
  
  #DOA
  micronesia_hammerhead$DOA <- rowMeans(micronesia_hammerhead_DOA[,1:1000])
  micronesia_hammerhead$DOA.sd = apply(micronesia_hammerhead_DOA[,1:1000], 1, sd)
  
  #prm
  micronesia_hammerhead$PRM = rowMeans(micronesia_hammerhead_PRM[,1:1000])
  micronesia_hammerhead$PRM.sd = apply(micronesia_hammerhead_PRM[,1:1000], 1, sd)
  
  #total
  micronesia_hammerhead$total = rowMeans(micronesia_hammerhead_total[,1:1000])
  micronesia_hammerhead$total.sd = apply(micronesia_hammerhead_total[,1:1000], 1, sd)
  
  #### othersharks sharks ####
  #row 10 in mortality; need to change this for each species when drawing rates)
  r_othersharks = 10
  
  micronesia_othersharks <- micronesia_comb[,c(1:4,33,34)] #create dataframe
  
  micronesia_othersharks_catches <- matrix(nrow=n, ncol=1000)
  micronesia_othersharks_DOA <- matrix(nrow=n, ncol=1000)
  micronesia_othersharks_released <- matrix(nrow=n, ncol=1000)
  micronesia_othersharks_PRM<- matrix(nrow=n, ncol=1000)
  micronesia_othersharks_total <- matrix(nrow=n, ncol=1000)
  
  micronesia_othersharks_CM_rate <- runif(1000,mortality$HookMin[r_othersharks],mortality$HookMax[r_othersharks])
  micronesia_othersharks_PRM_rate <- rlogitnorm(1000, mortality$logit.prm[r_othersharks],mortality$logit.prm.se[r_othersharks])
  
  for (j in 1:n) {
    for (i in 1:1000) {
      micronesia_othersharks_catches[j,i] <- micronesia_hook_preds[j,i]/1000 * rlnorm(1, micronesia_comb$othersharks.logcpue[j], micronesia_comb$othersharks.logcpue.se[j])
      micronesia_othersharks_DOA[j,i] <- micronesia_othersharks_catches[j,i] * micronesia_othersharks_CM_rate[i]
      micronesia_othersharks_released[j,i] <- micronesia_othersharks_catches[j,i] - micronesia_othersharks_DOA[j,i]
      micronesia_othersharks_PRM[j,i] <- micronesia_othersharks_released[j,i] * micronesia_othersharks_PRM_rate[i]
      micronesia_othersharks_total[j,i] <- micronesia_othersharks_DOA[j,i] + micronesia_othersharks_PRM[j,i]
    }
  }
  
  #run averages
  micronesia_othersharks_proj <- data.frame("Species" = "Other Sharks",
                                    "catch" = NA,
                                    "catch.upr" = NA,
                                    "catch.lwr" = NA,
                                    "DOA" = NA,
                                    "DOA.upr" = NA,
                                    "DOA.lwr" = NA,
                                    "PRM" = NA,
                                    "PRM.upr" = NA,
                                    "PRM.lwr" = NA,
                                    "total" = NA,
                                    "total.upr" = NA,
                                    "total.lwr" = NA)
  
  micronesia_othersharks_total_catches <- colSums(micronesia_othersharks_catches)
  micronesia_othersharks_proj$catch <- mean(micronesia_othersharks_total_catches)
  micronesia_othersharks_proj$catch.upr <- quantile(micronesia_othersharks_total_catches, 0.95)
  micronesia_othersharks_proj$catch.lwr <- quantile(micronesia_othersharks_total_catches, 0.05)
  
  micronesia_othersharks_total_DOA <- colSums(micronesia_othersharks_DOA)
  micronesia_othersharks_proj$DOA <- mean(micronesia_othersharks_total_DOA)
  micronesia_othersharks_proj$DOA.upr <- quantile(micronesia_othersharks_total_DOA, 0.95)
  micronesia_othersharks_proj$DOA.lwr <- quantile(micronesia_othersharks_total_DOA, 0.05)
  
  micronesia_othersharks_total_PRM <- colSums(micronesia_othersharks_PRM)
  micronesia_othersharks_proj$PRM <- mean(micronesia_othersharks_total_PRM)
  micronesia_othersharks_proj$PRM.upr <- quantile(micronesia_othersharks_total_PRM, 0.95)
  micronesia_othersharks_proj$PRM.lwr <- quantile(micronesia_othersharks_total_PRM, 0.05)
  
  micronesia_othersharks_total_total <- colSums(micronesia_othersharks_total)
  micronesia_othersharks_proj$total <- mean(micronesia_othersharks_total_total)
  micronesia_othersharks_proj$total.upr <- quantile(micronesia_othersharks_total_total, 0.95)
  micronesia_othersharks_proj$total.lwr <- quantile(micronesia_othersharks_total_total, 0.05)
  
  #cell averages
  #catches
  micronesia_othersharks$catch <- rowMeans(micronesia_othersharks_catches[,1:1000])
  micronesia_othersharks$catch.sd = apply(micronesia_othersharks_catches[,1:1000], 1, sd)
  
  #DOA
  micronesia_othersharks$DOA <- rowMeans(micronesia_othersharks_DOA[,1:1000])
  micronesia_othersharks$DOA.sd = apply(micronesia_othersharks_DOA[,1:1000], 1, sd)
  
  #prm
  micronesia_othersharks$PRM = rowMeans(micronesia_othersharks_PRM[,1:1000])
  micronesia_othersharks$PRM.sd = apply(micronesia_othersharks_PRM[,1:1000], 1, sd)
  
  #total
  micronesia_othersharks$total = rowMeans(micronesia_othersharks_total[,1:1000])
  micronesia_othersharks$total.sd = apply(micronesia_othersharks_total[,1:1000], 1, sd)
  
  #### micronesia Totals ####
  
  micronesia_bsh$Species = "Blue Shark"
  micronesia_silky$Species = "Silky Shark"
  micronesia_thresher$Species="Thresher"
  micronesia_mako$Species="Mako"
  micronesia_oceanicwhitetip$Species="Oceanic Whitetip"
  micronesia_hammerhead$Species="Hammerhead"
  micronesia_othersharks$Species="Other Sharks"
  
  colnames(micronesia_bsh)[c(5,6)] <- c("logcpue","logcpue.se")
  colnames(micronesia_silky)[c(5,6)] <- c("logcpue","logcpue.se")
  colnames(micronesia_thresher)[c(5,6)] <- c("logcpue","logcpue.se")
  colnames(micronesia_mako)[c(5,6)] <- c("logcpue","logcpue.se")
  colnames(micronesia_oceanicwhitetip)[c(5,6)] <- c("logcpue","logcpue.se")
  colnames(micronesia_hammerhead)[c(5,6)] <- c("logcpue","logcpue.se")
  colnames(micronesia_othersharks)[c(5,6)] <- c("logcpue","logcpue.se")
  
  micronesia_total_preds <- rbind(micronesia_bsh,
                             micronesia_silky,
                             micronesia_thresher,
                             micronesia_mako,
                             micronesia_oceanicwhitetip,
                             micronesia_hammerhead,
                             micronesia_othersharks)
  
  end.time <- Sys.time()
  end.time - start.time}

write.csv(micronesia_total_preds, "micronesia_total_preds.csv", row.names = F)

#### summary ####
micronesia_summary <- rbind(micronesia_bsh_proj,
                       micronesia_silky_proj,
                       micronesia_thresher_proj,
                       micronesia_mako_proj,
                       micronesia_oceanicwhitetip_proj,
                       micronesia_hammerhead_proj,
                       micronesia_othersharks_proj)

write.csv(micronesia_summary, "micronesia_summary.csv", row.names = F)

#Plotting gor fig1
setwd('~/Desktop/Ongoing Projects/GFW Shark Sanctuary/working_dir')
micronesia_mortality <- read.csv("micronesia_total_preds.csv")
micronesia_proj <- aggregate(data=micronesia_mortality, total ~ Lat + Long, FUN='sum')
dev.off()
world <- map_data("world")
micronesia_map <- ggplot(data=micronesia_proj, aes(x=Long, y=Lat)) +
  geom_tile(aes(fill=total)) +
  scale_fill_gradient(name="Bycatch Mortality \n(# of individuals)", low="khaki1", high="red",
                      breaks=c(0,50,100,150,200),
                      labels=c(0,50,100,150,200),
                      limits=c(0,200)) +
  geom_polygon(data = fsm_shp, aes(x = long, y = lat, group = group), color="black", fill=NA, size = 0.6) +
  geom_map(data = world, map = world, aes(long, lat, map_id = region)) +
  scale_x_continuous(limits=c(134,167), breaks=seq(135,165,by=5)) +
  scale_y_continuous(limits=c(-3,14.5), breaks=seq(0,10,by=5)) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        panel.background = element_rect(fill="white", colour = "black"),
        panel.grid.major = element_line(colour="grey97")) +
  labs(subtitle="Federated States of Micronesia")
