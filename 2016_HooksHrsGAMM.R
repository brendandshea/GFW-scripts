...
datMaster2 <- subset(datMaster2, year==2016)
dat <- read.csv("~/Downloads/GFW_AIS/2016LL.csv") #AIS data at 5x5 res
...
# GAMM model ####
#write model
gamm1 = gam(log(hooks2)~log(hours)+s(lat)+s(lon, k=18)+s(fleet, bs = "re"), data = tot2, method='REML', family=scat) 
...
#save it
saveRDS(gamm1, "hooks_gamm_2016.rds")
...
write_csv(aisdat, "~/Desktop/Ongoing Projects/GFW Shark Sanctuary/working_dir/2016effort_*25x*25.csv")
